...
capdata_week<- read.table("Activation_week_1.txt", header=TRUE)   ### Weekly activations
capdata_day<- read.table("Activation_day_1.txt", header=TRUE) ### Daily activations

##Descriptive outputs
capdata_week_des<-descriptive(capdata_week[,2:77],dfreq=FALSE)
#write.table(capdata_week_des$base.freq, file = "capdata_week_des_FL.txt")
