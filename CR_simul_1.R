#' Simulation study in a mark-recapture experiment
#' 
#' This function produces Monte Carlo estimates and bootstrap variances of demographic parameters and capture probabilities per PSP 
#' under model Mh robust design
#' 
#' @param n.weeks An inetegr. The total number of weeks.
#' @param n.entry An integer. The total number of entries per week
#' @param phi The survival probabily per PSP.
#' @param betai A value for the beta parameter.
#' @param taui A valur for the heterogeneity parameter.
#' @param vt The number of PSP.
#' @param J The number of SSP within each of the vt PSPs. 
#' @param trials The number of repetitions for the Monte Carlo study.
#' @param trialsB The number of bootstrap repetitions within each Monte Carlo repetition.
#'                                                                                                                                                    
#' @return \item{results}{A list containing}
#' \itemize{
#' \item{SampleEstisim}{A c(vt,2,trials) dimensional array containing the simulation samples}
#' \item{SampleEstiBoot}{A vector of trialsB elements containing the bootstrap estimate of the variance for N5}
#' \item{var_N}{A c(vt,10) dimensional matrix containing RB,RRMSE,RLCI,...}
#' }
#' 
#' @author XXXX
#'
...
#' 
#' @examples
#' CR_simul_1(n.weeks,n.entry,phi,betai,taui,vt,J,trials,trialsB)
CR_simul_1<-function(n.weeks,n.entry,phi,betai,taui,vt,J,trials,trialsB){
  ...
  for(k in 1:trials){
    ...
    CR_out<-smartP_simul_LP_2_1(n.weeks,n.entry,phi,betai,taui)
    ...
    for(i in 1:vt){
      ...
      # cdp1<-closedp.0(DT[,1:7],dfreq=FALSE,dtype="hist")
  ...
  #return(var_N)
  # return(SampleEsti)
  results<- list(SampleEstisim=SampleEsti,SampleEstiBoot=SampleEstiBoot,var_N=var_N)
  return(results)
}
